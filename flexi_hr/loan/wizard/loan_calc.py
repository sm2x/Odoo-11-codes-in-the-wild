    ...
    def get_payment_data(self):
        ...
        if self._context.get('active_id'):
            ...
            for line in self.loan_calc_line_ids:
                sal_payment_ids = self.env['salary.payment'].search([('employee_id', '=', loan_app_rec.employee_id.id),
                                                                    ('state', '=', 'deducted')])
                sal_amount = 0.0
                each_due_date = datetime.datetime.strptime(line.due_date, '%Y-%m-%d').strftime('%m-%Y')
                for each_sal_payment in sal_payment_ids:
                    each_sal_date = datetime.datetime.strptime(each_sal_payment.date, '%Y-%m-%d').strftime('%m-%Y')
                    if each_sal_date == each_due_date \
                            and loan_app_rec.employee_id.contract_id \
                            and loan_app_rec.employee_id.contract_id.wage:
                        sal_amount += each_sal_payment.amount
                if sal_amount > 0 and line.total > loan_app_rec.employee_id.contract_id.wage + sal_amount:
