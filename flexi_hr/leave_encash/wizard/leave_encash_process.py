...
class leave_encash_process(models.TransientModel):
    ...
    def search_employee(self):
        ...
        for emp in emp_ids:
            ...
            leave_status_add = self.env['hr.holidays'].sudo().search([('employee_id.id','=',emp.id),
                                                               ('type','=','add'),
                                                               ('holiday_status_id', '=', self.leave_type_id.id),
                                                               ('state', '=', 'validate')])
            leave_status_remove = self.env['hr.holidays'].sudo().search([('employee_id.id','=',emp.id),
                                                                  ('type','=','remove'),
                                                                  ('state', '=', 'validate'),
                                                                  ('holiday_status_id', '=', self.leave_type_id.id)])
            ...
            for add in leave_status_add:
                allocated_days += add.number_of_days_temp
            for remove in leave_status_remove:
                removed_days += remove.number_of_days_temp
            total_leave_pending = allocated_days - removed_days
            encashed_employee = self.env['leave.encash'].sudo().search([
                                                     ('employee_id','=',emp.id),
    ...
    def search_leave(self):
        ...
        holiday_status_ids = self.env['hr.holidays.status'].sudo().search([])
        leave_encash_settings = self.env['res.config.settings'].sudo().search([],limit=1,order='id DESC')
        list = []
        if self.leave_type_id:
            ...
            for employee in employee_ids:
                    ...
                    removed_days = 0
                    total_leave_pending = 0
                    leave_status_add = self.env['hr.holidays'].sudo().search([('employee_id.id','=',employee.id),
                                                                           ('holiday_status_id','=',each_status.id),
                                                                           ('type','=','add'),
                                                                           ('state', '=', 'validate'),
                                                                         ])
                    leave_status_remove = self.env['hr.holidays'].sudo().search([('employee_id.id','=',employee.id),
                                                                              ('holiday_status_id','=',each_status.id),
                                                                              ('type','=','remove'),
                                                                              ('state', '=', 'validate'),
                                                                            ])
            ...
            for each_status in holiday_status_ids:
                for employee in employee_ids:
                    ...
                    removed_days = 0
                    total_leave_pending = 0
                    leave_status_add = self.env['hr.holidays'].sudo().search([
                                                                       ('employee_id.id','=',employee.id),
                                                                       ('holiday_status_id','=',each_status.id),
                                                                       ('type','=','add'),
                                                                       ('state', '=', 'validate'),
                                                                    ])
                    leave_status_remove = self.env['hr.holidays'].sudo().search([
                                                                          ('employee_id.id','=',employee.id),
                                                                          ('holiday_status_id','=',each_status.id),
                                                                          ('type','=','remove'),
                                                                          ('state', '=', 'validate'),
